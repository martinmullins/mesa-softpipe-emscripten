...
function write_canvas(buffer,Width,Height){ console.log('hey lets export', buffer); var imageData = new Uint8Array(HEAPU8.subarray(buffer, buffer+Width*Height*4)); var c = document.getElementById("canvas"); c.width = Width; c.height = Height; var ctx = c.getContext("2d"); var imgData = ctx.createImageData(Width, Height); var i,j,y,x; for (y=Height-1; y>=0; y--) { for (x=0; x<Width; x++) { i = (y*Width + x) * 4; j = ((Height - y - 1)*Width + x) * 4; imgData.data[j+0] = imageData[i]; imgData.data[j+1] = imageData[i+1]; imgData.data[j+2] = imageData[i+2]; imgData.data[j+3] = 255; } } ctx.putImageData(imgData, 0, 0); return; }
